# Extract medication request data from FHIR MedicationRequest resource and format for SQLite insertion
{
  ...
  status: .status,
  intent: .intent,
  medication_id: (if .medicationReference.reference then .medicationReference.reference | split("/")[1] else null end),
  ...
  requester_id: (if .requester.reference then .requester.reference | split("/")[1] else null end),
